...
public abstract class Entity {
    ...
    public int rightedge(Entity other){
        return other.getX() + other.getWidth();
    }

    public int leftedge(Entity other){
        return other.getX();
    }

    public int bottom(Entity other){
        return other.getY() + other.getHeight();
    }

    public int top(Entity other){
        return other.getY();
    }
    ...
    public boolean isCollidingWith(Entity other){
        if (getX() > rightedge(other)){
            return false;
        }
        else if(getX() + getWidth() < leftedge(other)) {
            return false;
        }
        else if(getY() > bottom(other)) {
            return false;
        }
        else if(getY() + getHeight() < top(other)) {
            return false;
        }
        else 
            return true;
